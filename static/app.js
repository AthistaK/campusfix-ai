...
async function openStudentProfile() {
    ...
    const name = prompt(
        "Enter your name",
        localStorage.getItem("campusfix_student_name") || ""
    );
    ...
    const res = await fetch(
        `/api/students/${encodeURIComponent(studentName)}`
    );

    const data = await res.json();

    if (!res.ok) {
        toast(data.error || "Unable to load profile.");
        return;
    }
    ...
    document.querySelector(".modal-card").innerHTML = `
        ...
        <div class="profile-card">
            ...
            <div class="profile-item">
                <small>Course</small>
                <strong>
                    ${escapeHtml(data.student.course || "CSE")}
                </strong>
            </div>
            ...
            <div class="profile-item">
                <small>Year</small>
                <strong>
                    ${escapeHtml(data.student.year || "1st Year")}
                </strong>
            </div>
            ...
            <div class="profile-item">
                <small>Email</small>
                <strong>
                    ${escapeHtml(data.student.email || "Not added")}
                </strong>
            </div>

        </div>

        <p class="eyebrow" style="margin-top:28px;">
            MY REPORTS
        </p>

        <h3>
            ${data.reports.length} reported issue${data.reports.length === 1 ? "" : "s"}
        </h3>
            ...
            ${
                ...
                data.reports.map(r => `
                    ...
                    <div class="my-report">
                        ...
                        <div class="my-report-right">

                            <span class="priority ${severityClass(r.severity)}">
                                ${r.priority}/100
                            </span>
